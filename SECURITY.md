...
- The YouTube key remains in the server environment. Claude uses the local Claude Code sign-in; Higgsfield uses its own OAuth session that Claude Code stores (created with `/mcp`). The server stores neither, and it removes `ANTHROPIC_API_KEY` from the `claude` child environment.
